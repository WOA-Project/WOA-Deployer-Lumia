...
namespace Deployer.Services
{
        ...
        where TArchive : AbstractArchive<TEntry, TVolume>
    {
        ...
        {
            Log.Verbose("Extracting from {File}", archivePath);
            ...
            Log.Verbose("Extraction successful from {Path}", archivePath);
        }

        public async Task<string> ReadToEnd(string archivePath, string key)
        {
            Log.Verbose("Importing Core Package from {File}", archivePath);
